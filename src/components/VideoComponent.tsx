...
const VideoComponent: React.FC<VideoComponentProps> = ({ src, poster, overlay, position = "random" }) => {
  const [randomPosition, setRandomPosition] = useState<{ top: string; left: string }>({ top: '0%', left: '0%' });

  useEffect(() => {
    if (position === "random") {
      // Generate random positions only when the component mounts
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: overlay ? 0 : 15, // Set z-index based on overlay prop
      }}
    >
      <video
