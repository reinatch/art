...
const ScrollingBoxes: React.FC<ScrollingBoxesProps> = ({ children }) => {
  ...
  return (
    <>
      <div ref={wrapperRef} className="wrapper relative mx-auto  w-full h-auto">
        <div ref={boxesRef} className="boxes relative flex gap-4 h-10">
          {React.Children.map(children, (child, index) => (
            <div
              key={index}
              className="box relative h-auto  text-black whitespace-nowrap"
            >
              {" "}
              {/* Added whitespace-nowrap for responsiveness */}
              {child}
            </div>
          ))}
        </div>
