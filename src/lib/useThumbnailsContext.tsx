...
const ThumbnailsContext = createContext<ThumbnailsContextType | undefined>(
  undefined
);
export const ThumbnailsProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
export const useThumbnailsContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useThumbnailsContext must be used within a ThumbnailsProvider"
    );
  }
  return context;
};
