...
import { useThumbnailsContext } from "@/lib/useThumbnailsContext"; // New import
import Image from "next/image";
// import useHorizontalScrollSnap from "@/utils/useHorizontalSnap";
// import useHorizontalSliderSnap from "./horizontalSliderSnap";
...
// Define an interface for the gallery images
...
const HorizontalSnapSlider: React.FC<HorizontalSnapSliderProps> = ({ galeria, video }) => {
  const scrollContainerRef = useRef<HTMLDivElement | null>(null);
  const { setThumbnails, selectedThumbnail, setSelectedThumbnail, thumbRefs } = useThumbnailsContext(); // Use thumbnails context
...
useEffect(() => {
  ...
  ? [{ id: 0, url: "/images/frames/Video-icon_01.png" }, ...galeria.map(image => ({ id: image.ID, url: image.url }))]
  : galeria.map(image => ({ id: image.ID, url: image.url }));
    // Set thumbnails whenever galeria changes
...
useGSAP(() => {
  // Define the sections and initialize the snapping points


  gsap.registerPlugin(ScrollTrigger,ScrollSmoother, ScrollToPlugin, Observer, useGSAP);
...
mm.add("(min-width: 700px)", () => {
  ...
  //     pin: true,
  //     scrub: 1,
  //     snap: 1 / (sectionEls.length - 1),
  //     end: `+=${totalWidth}`,
  //   }
    
  // })
  // tl.to(sectionEls, {
  //   xPercent: -101 * (sectionEls.length - 1),
  //   ease: 'none'
  // }, 0)
      ...
      >
         {video && (
                ...
                <div className=" w-screen h-[65vh]">
                  ...
                  <div id="0" className="video_wrap h-full w-screen max-h-[30vh]" ref={el => { thumbRefs.current[0] = el; }}>
                {/* <iframe
                  id="videoIframe"
                ...
                ></iframe> */}
                        ...
                        <div className="relative pt-[56.25%] md:h-[65vh] w-[65vw]">
                          ...
                          src={videoSrc + "&autoplay=1&controls=0&title=0&byline=0&portrait=0&muted=1&loop=1" || ""}
