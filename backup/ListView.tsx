...
interface ListViewProps {
  ...
  hoveredProjectId: number | null;
  containerRef: React.RefObject<FixedSizeList<Projecto[]> | null>;
}
  ...
  const getListData = () => {
    switch(selectedFilter) {
      case 'artistas': return filteredArtistProjects;
      case 'materiais': return filteredMaterialProjects;
      case 'ano': return filteredYearProjects.flatMap(group => group.projects);
      default: return [];
    }
  };
  ...
  const listData = getListData();
    const FilterList: React.FC<FilterListProps> = ({
      ...
      const ProjectListItem = React.memo(({ index, style, data }: { 
        index: number, 
        style: React.CSSProperties, 
        data: Projecto[] 
      }) => {
        const projecto = data[index];
        return (
          <div 
            style={style}
...
return (  <div className="flex w-full h-[70dvh]">
  ...
  {loading && ((artists?.length ?? 0) === 0 || (years?.length ?? 0) === 0 || (materials?.length ?? 0) === 0) ? (
    ...
    <>
     ...
     <div className="w-1/2 pl-4 pb-[12vh] h-full">
        ...
        <AutoSizer>
          {({ height, width } : Size) => (
            <List
              ...
              // itemSize={50} // Adjust based on your item height
              itemData={listData}
              overscanCount={5} // Pre-render extra items
              ref={containerRef as React.RefObject<FixedSizeList<Projecto[]> | null>}
            >
              {ProjectListItem}
            </List>
          )}
